...
```bash
# This command uses your 'mcp-ee.yaml' file to build an image
# and tags it as 'mcp-ee:latest'

ansible-builder build --file mcp-ee.yaml --tag mcp-ee:latest --container-runtime podman
```
...
```bash
ansible-navigator run playbook.yml --eei mcp-ee:latest --pull-policy missing
```
...
podman tag mcp-ee:latest your-pah-hostname/mcp-ee:latest

# Login to your Private Automation Hub
podman login your-pah-hostname

# Push the image
podman push your-pah-hostname/mcp-ee:latest
```
...
podman tag mcp-ee:latest your-internal-registry.com/mcp-ee:latest
...
If you want to reuse this execution environment template for future projects, you can import the generated `mcp-ee.yaml` file into your self-service automation portal.
...
2. **Import from Git repository**: Enter the Git SCM URL containing your `mcp-ee.yaml` file, click **Analyze** to validate, review the details, then click **Import**.
